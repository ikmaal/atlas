    ...
    container.innerHTML = changesets.map(cs => {
        ...
        if (cs.validation) {
            ...
            if (validation.status === 'suspicious') {
                ...
                validationHTML = `<span class="badge badge-suspicious" title="${escapeHtml(reasons)}">⚠️ Suspicious</span>`;
            } else if (validation.status === 'warning') {
                const reasons = validation.reasons.join(', ');
                validationHTML = `<span class="badge badge-warning" title="${escapeHtml(reasons)}">⚡ Warning</span>`;
            } else {
...
function updateLegend() {
    ...
    legendContent.innerHTML = `
        ...
        <div class="legend-item interactive ${validationVisibility.warning ? 'active' : 'inactive'}" onclick="toggleValidationVisibility('warning')">
            <span class="legend-color" style="background: #f59e0b;"></span>
            <span class="legend-label">⚡ Warning</span>
            <span class="legend-toggle">${validationVisibility.warning ? '👁️' : '👁️‍🗨️'}</span>
        </div>
        <div class="legend-item interactive ${validationVisibility.suspicious ? 'active' : 'inactive'}" onclick="toggleValidationVisibility('suspicious')">
            <span class="legend-color" style="background: #ef4444;"></span>
            <span class="legend-label">⚠️ Suspicious</span>
            <span class="legend-toggle">${validationVisibility.suspicious ? '👁️' : '👁️‍🗨️'}</span>
        </div>
    `;
}
...
function displayUserProfile(user, stats) {
    ...
    if (total > 0) {
        ...
        const warningPct = ((stats.validation.warning / total) * 100).toFixed(1);
        const suspiciousPct = ((stats.validation.suspicious / total) * 100).toFixed(1);
        
        document.getElementById('profileValidCount').textContent = 
            `${stats.validation.valid} (${validPct}%)`;
        document.getElementById('profileWarningCount').textContent = 
            `${stats.validation.warning} (${warningPct}%)`;
        document.getElementById('profileSuspiciousCount').textContent = 
            `${stats.validation.suspicious} (${suspiciousPct}%)`;
    }
    ...
    if (stats.recent_changesets && stats.recent_changesets.length > 0) {
        stats.recent_changesets.forEach(cs => {
            ...
            const validationClass = validationStatus === 'valid' ? 'badge-valid' : 
                                   validationStatus === 'warning' ? 'badge-warning' : 'badge-suspicious';
            const validationText = validationStatus === 'valid' ? '✓ Valid' :
                                  validationStatus === 'warning' ? '⚡ Warning' : '⚠️ Suspicious';
